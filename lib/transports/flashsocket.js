...
function FlashWS(options){
  ...
  this.policyAddress = options.policyAddress;
  this.forceFlash = options.forceFlash;
}
...
FlashWS.prototype.ready = function(fn){
  if (!WebSocket || !( '__initialize' in WebSocket) || !global.swfobject) {
    return;
  }
  ...
  function init () {
    ...
    if (!FlashWS.loaded) {
      // if policy address is not provided, we asume it's provided by the same host
      var policy = 'xmlsocket://' + (self.policyAddress ? self.policyAddress : self.hostname) + ':' + self.policyPort;
      debug('loadFlashPolicyFile "%s"', policy);
...
FlashWS.prototype.check = function(){
  ...
  // TODO Move flashsocket feature detection into a separate module hasFlash -> hasFlash
  if (!this.forceFlash && WebSocket && !( '__initialize' in WebSocket)) {
    return false;
  }
